...
export default async function Page({params,searchParams}:{params:Promise<{appointmentId:string}>,searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <div>
    ...
    <form action={saveConsultationAction} className="card section-card attendance-form">
      ...
      <label>Queixa principal<textarea name="complaint" rows={3} defaultValue={c?.complaint??""}/></label>
      <label>Anamnese<textarea name="anamnesis" rows={6} defaultValue={c?.anamnesis??""}/></label>
      <label>Exame físico / avaliação objetiva<textarea name="examination" rows={5} defaultValue={c?.examination??""}/></label>
      <label>Avaliação clínica / hipótese<textarea name="assessment" rows={4} defaultValue={c?.assessment??""}/></label>
      <label>Evolução<textarea name="evolution" rows={6} defaultValue={c?.evolution??""}/></label>
