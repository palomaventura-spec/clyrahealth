...
export default async function Page({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  const professionalFilter=user.role==="PROFESSIONAL"&&user.professional?.id?user.professional.id:undefined;
    ...
    <section className="card section-card"><div className="table-wrap"><table>
      ...
      <tbody>{appointments.map(a=><tr key={a.id}>
        ...
        <td><div className="table-actions">
          ...
          {user.role==="PROFESSIONAL"&&user.professional?.id===a.professionalId&&["SCHEDULED","CONFIRMED","ARRIVED"].includes(a.status)&&<form action={startConsultationAction}><input type="hidden" name="appointmentId" value={a.id}/><button className="btn btn-small btn-primary">Iniciar atendimento</button></form>}
          {user.role==="PROFESSIONAL"&&user.professional?.id===a.professionalId&&a.status==="IN_PROGRESS"&&<Link href={`/atendimentos/${a.id}`} className="btn btn-small btn-primary">Continuar</Link>}
          {user.role==="PROFESSIONAL"&&user.professional?.id===a.professionalId&&a.status==="COMPLETED"&&<Link href={`/atendimentos/${a.id}`} className="btn btn-small btn-secondary">Revisar</Link>}
        </div></td>
